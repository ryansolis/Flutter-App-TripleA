...
class _Body extends State<Body> {
  ...
  Future<List> _login() async {
    final response =
        await http.post("https://10.0.2.2/triple_a/login.php", body: {
    ...
    if (datauser.length == 0) {
      ...
      if (datauser[0]['level'] == 'admin') {
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
                builder: (BuildContext context) => AdminBulletinBoard()));
      } else if (datauser[0]['level'] == 'student') {
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
                builder: (BuildContext context) => StudentBulletinBoard()));
      } else if (datauser[0]['level'] == 'personnel') {
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
                builder: (BuildContext context) => PersonnelBulletinBoard()));
      }
    }
  }
